...
git checkout dca3fd3  # 動作確認済みバージョンに固定
python -m venv .venv
...
`.mcp.json` をプロジェクトルートに作成するか、`~/.claude/.mcp.json`（グローバル設定）に追加する。

> **注意**: `.mcp.json` には API キーが含まれます。このリポジトリの `.gitignore` で除外済みですが、他のプロジェクトで使う場合は `git add` しないよう注意してください。
